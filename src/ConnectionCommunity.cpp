...
ConnectionCommunity::CreateIconList(HWND hwndmain, wyUInt32 *numicons)
{
	...
	wyUInt32    ids[] = {
		...
		ID_TRANSACTION_SETAUTOCOMMIT,	ID_TRX_REPEATABLEREAD,			
		ID_TRX_READCOMMITED,			ID_TRX_READUNCOMMITED,
		ID_TRX_SERIALIZABLE,			ID_WITHCONSISTENTSNAPSHOT_READONLY,
		ID_WITHCONSISTENTSNAPSHOT_READWRITE,
		ID_STARTTRANSACTION_READONLY,	ID_STARTTRANSACTION_READWRITE,
		ID_COMMIT_ANDCHAIN,				ID_COMMIT_ANDNOCHAIN,
		ID_COMMIT_RELEASE,				ID_COMMIT_NORELEASE,
...
ConnectionCommunity::CreateSourceInstance(CopyDatabase *copydb)
{
	...
	if(copydb->m_srcinfo->m_initcommand.GetLength())
		ExecuteInitCommands(tempmysql, copydb->m_newsrctunnel, copydb->m_srcinfo->m_initcommand);
...
ConnectionCommunity::CreateTargetInstance(CopyDatabase *copydb)
{
	...
	if (copydb->m_tgtinfo->m_initcommand.GetLength())
		ExecuteInitCommands(tempmysql, copydb->m_newtargettunnel, copydb->m_tgtinfo->m_initcommand);
